...
class DANN(nn.Module):
    ...
    def __init__(self, input_channels=2, num_classes=12):
        ...
        self.label_classifier = LabelClassifier(self.feature_extractor.feature_dim, num_classes)
        self.domain_discriminator = DomainDiscriminator(self.feature_extractor.feature_dim)
...
def main():
    ...
    test_case_path = r'D:\RadioData\testcase0715'  # 目标域测试集（有标签）
    ...
    # 创建模型和训练器
    model = DANN(input_channels=2, num_classes=10)
    ...
    if target_test_loader is not None:
        ...
        # 加载最佳模型
        checkpoint = torch.load('best_dann_model10.pth')
